...
namespace HydroDemo
{
    public partial class Form1 : Form
    {
                ...
                for (int i = 0; i < posts.Count; i++)
                {
                    ...
                    if (yordamchi != null && yordamchi.Count > 0)
                    {
                        for (int j = 0; j < yordamchi.Count; j++)
                        {
                            ...
                            //MessageBox.Show(tfor_pdk[4] + " = " + yordamchi[j].Namlik);
                            ...
                            if (tfor_pdk[31] && yordamchi[j].Eh_MB != -1)
                            {
                                ...
                                if (yordamchi[j].Eh_MB < ob.list[31].min)
                                {
                                    ob.list[31].min = yordamchi[j].Eh_MB;
                                }
                                if (yordamchi[j].Eh_MB > ob.list[31].max)
                                {
                                    ob.list[31].max = yordamchi[j].Eh_MB;
                                }
                            }
                            ...
                            if (tfor_pdk[33] && yordamchi[j].FeUmumiy != -1)
                            {
                                ...
                                if (yordamchi[j].FeUmumiy > ob.list[33].max)
                                {
                            ...
                            if (tfor_pdk[58] && yordamchi[j].Butifos != -1)
                            {
                                ...
                                if (yordamchi[j].Butifos < ob.list[58].min)
                                {
                                    ob.list[58].min = yordamchi[j].Butifos;
                                }
                                if (yordamchi[j].Butifos > ob.list[58].max)
                                {
                ...
                for (int i = 0; i < posts.Count; i++)
                {
                    ...
                    if (yordamchi != null && yordamchi.Count > 0)
                    {
                        for (int j = 0; j < yordamchi.Count; j++)
                        {
                            ...
                            {
                                ob.list[10].umumiy++;
                                yordamchi[j].O2 = koms[10].PDK / yordamchi[j].O2;
                                ob.list[10].ortacha += yordamchi[j].O2;
                                if (yordamchi[j].O2 < ob.list[10].min)
                                {
                                    ob.list[10].min = yordamchi[j].O2;
        ...
        private void иЗВПоРекамToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                int year = 0;
                ...
                IZVSelectYear selectYear = new IZVSelectYear(rivers);
                selectYear.ShowDialog();

                year = selectYear.year;
                riverId = selectYear.riverId;
                ...
                {
                    query = $"Select * from Analysis Where Sana >#31/12/{year - 1}#" +
                    $"AND Sana<#01/01/{year}#";
                }

                List<AnalysisClass> list = this.GetAnalysisList(query);
                List<AnalysisClass> result = new List<AnalysisClass>();
                posts.Where(x => x.River_Id == riverId).ToList().ForEach(p =>
                {
        ...
        private void картыToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                string strquery = "Select *from Map";
                this.connect.Close();
                this.connect.Open();
                this.command.CommandText = strquery;
                this.adapter.InsertCommand = this.command;
                ...
                this.connect.Close();
                List<HydroSoft.Models.MapModel> maps = new List<HydroSoft.Models.MapModel>();

                for (int index = 0; index < table.Rows.Count; ++index)
                {
                    HydroSoft.Models.MapModel model = new HydroSoft.Models.MapModel();
                    int result1 = 0;
                    model.Id = int.TryParse(table.Rows[index].ItemArray[0].ToString(), out result1) ? result1 : 0;
                    model.Name = table.Rows[index].ItemArray[1].ToString();

                    maps.Add(model);
                }

                HydroSoft.Forms.Map map = new HydroSoft.Forms.Map(maps, createImage, deleteImage);
                map.Show();
            }
            catch (Exception exc)
            {
        ...
        public void createImage(HydroSoft.Models.MapModel map, string file)
        {
            try
            {
                // Copying images into images foder with naming of map id
                if (File.Exists(file))
                {
                    this.connect.Close();
                    this.connect.Open();
                    string query = $"Insert Into[Map]([Name]) Values('{map.Name}')";
                    this.command.CommandText = query;
                    this.command.ExecuteNonQuery();
                    this.command.CommandText = "Select Max(Id) From Map";
                    map.Id = (int)this.command.ExecuteScalar();

                    string image = Directory.GetCurrentDirectory() + $"/images/{map.Id}";
                    File.Copy(file, image);
                    this.connect.Close();
                }
                else
        ...
        public void deleteImage(HydroSoft.Models.MapModel map)
        {
            try
            {
                string image = Directory.GetCurrentDirectory() + $"/images/{map.Id}";
                ...
                if (File.Exists(image))
                {
                    this.connect.Close();
                    this.connect.Open();
                    string query = $"Delete from [Map] Where Id={map.Id}";
                    this.command.CommandText = query;
                    this.command.ExecuteNonQuery();
